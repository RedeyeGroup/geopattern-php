...
class GeoPattern
{
    ...
    protected function geoHexagons()
    {
        ...
        for ($y = 0; $y <= 5; $y++) {
            for ($x = 0; $x <= 5; $x++) {
                ...
                $this->svg->addPolyline($hex, array_merge($styles, array('transform' => "translate($onePointFiveXSideLengthMinusHalfHexWidth, $dyMinusHalfHexHeight)")));

                // Add an extra one at top-right, for tiling.
                if ($x == 0) {
                    $onePointFiveSideLengthSixMinusHalfHexWidth = 6 * $sideLength * 1.5 - $hexWidth / 2;
                    $this->svg->addPolyline($hex, array_merge($styles, array('transform' => "translate($onePointFiveSideLengthSixMinusHalfHexWidth, $dyMinusHalfHexHeight)")));
                }
                ...
                if ($y == 0) {
                    ...
                    $this->svg->addPolyline($hex, array_merge($styles, array('transform' => "translate($onePointFiveXSideLengthMinusHalfHexWidth, $dy2MinusHalfHexHeight)")));
                }
                ...
                if ($x == 0 && $y == 0) {
                    ...
                    $this->svg->addPolyline($hex, array_merge($styles, array('transform' => "translate($onePointFiveSideLengthSixMinusHalfHexWidth, $fiveHexHeightPlusHalfHexHeight)")));
                }
    ...
    protected function geoSineWaves()
    {
        ...
        for ($i = 0; $i <= 35; ++$i) {
            ...
            $this->svg->addPath($str, array_merge($styles, array('transform' => "translate(-$quarterPeriod, $iWaveWidthMinusOnePointFiveAmplitude)")));
            $this->svg->addPath($str, array_merge($styles, array('transform' => "translate(-$quarterPeriod, $iWaveWidthMinusOnePointFiveAmplitudePlusThirtySixWaveWidth)")));
    ...
    protected function geoChevrons()
    {
        ...
        for ($y = 0; $y <= 5; ++$y) {
            for ($x = 0; $x <= 5; ++$x) {
                ...
                $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xChevronWidth,$yPointSixSixChevronHeightMinusHalfChevronHeight)")));
                // Add an extra row at the end that matches the first row, for tiling.
                if ($y == 0) {
                    $sixPointSixSixChevronHeightMinusHalfChevronHeight = 6 * $chevronHeight * 0.66 - $chevronHeight / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xChevronWidth,$sixPointSixSixChevronHeightMinusHalfChevronHeight)")));
                }
    ...
    protected function geoPlusSigns()
    {
        ...
        for ($y = 0; $y <= 5; $y++) {
            for ($x = 0; $x <= 5; $x++) {
                ...
                $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($t1, $t2)")));

                // Add an extra column on the right for tiling.
                if ($x == 0) {
                    $xT1 = 4 * $plusSize - $x * $squareSize + $dx * $squareSize - $squareSize;
                    $xT2 = $y * $plusSize - $y * $squareSize - $plusSize / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xT1, $xT2)")));
                }
                ...
                if ($y == 0) {
                    ...
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($yT1, $yT2)")));
                }
                ...
                if ($x == 0 && $y == 0) {
                    ...
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xyT1, $xyT2)")));
                }
    ...
    protected function geoXes()
    {
        ...
        for ($y = 0; $y <= 5; ++$y) {
            for ($x = 0; $x <= 5; ++$x) {
                ...
                $t2 = $dy - $y * $xSize / 2;
                $halfXSize = $xSize / 2;
                $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($t1, $t2) rotate(45, $halfXSize, $halfXSize)")));

                // Add an extra column on the right for tiling.
                if ($x == 0) {
                    $xT1 = 6 * $xSize / 2 - $xSize / 2;
                    $xT2 = $dy - $y * $xSize / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xT1, $xT2) rotate(45, $halfXSize, $halfXSize)")));
                }
                ...
                if ($y == 0) {
                    ...
                    $yT2 = $dy - 6 * $xSize / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($yT1, $yT2) rotate(45, $halfXSize, $halfXSize)")));
                }
                ...
                if ($y == 5) {
                    ...
                    $y2T2 = $dy - 11 * $xSize / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($y2T1, $y2T2) rotate(45, $halfXSize, $halfXSize)")));
                }
                ...
                if ($x == 0 && $y == 0) {
                    ...
                    $xyT2 = $dy - 6 * $xSize / 2;
                    $this->svg->addGroup($group, array_merge($styles, array('transform' => "translate($xyT1, $xyT2) rotate(45, $halfXSize, $halfXSize")));
                }
    ...
    protected function geoOctogons()
    {
        ...
        for ($y = 0; $y <= 5; $y++) {
            for ($x = 0; $x <= 5; $x++) {
                ...
                $this->svg->addPolyline($tile, array(
                    'fill' => $fill,
                    'fill-opacity' => $opacity,
                    'stroke' => self::STROKE_COLOR,
                    'stroke-opacity' => self::STROKE_OPACITY,
    ...
    protected function geoSquares()
    {
        ...
        for ($y = 0; $y <= 5; $y++) {
            for ($x = 0; $x <= 5; $x++) {
                ...
                $this->svg->addRectangle($x*$squareSize, $y*$squareSize, $squareSize, $squareSize, array(
                    'fill' => $fill,
                    'fill-opacity' => $opacity,
                    'stroke' => self::STROKE_COLOR,
                    'stroke-opacity' => self::STROKE_OPACITY,
    ...
    protected function geoConcentricCircles()
    {
        ...
        for ($y = 0; $y <= 5; $y++) {
            for ($x = 0; $x <= 5; $x++) {
                ...
                $this->svg->addCircle($cx, $cy, $halfRingSize, array(
                    'fill' => 'none',
                    'stroke' => $fill,
                    'style' => array(
                        'opacity' => $opacity,
                ...
                $this->svg->addCircle($cx, $cy, $quarterRingSize, array(
                    'fill' => $fill,
                    'fill-opacity' => $opacity,
                ));
    ...
    protected function geoTriangles()
    {
        ...
        for ($y = 0; $y <= 5; ++$y) {
            for ($x = 0; $x <= 5; ++$x) {
                ...
                $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($t1, $yTriangleHeight) rotate($rotation, $halfSideLength, $halfTriangleHeight)")));

                // Add an extra one at top-right, for tiling.
                if ($x == 0) {
                    $xT1 = 6 * $sideLength * 0.5 - $sideLength / 2;
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xT1, $yTriangleHeight) rotate($rotation, $halfSideLength, $halfTriangleHeight)")));
                }
    ...
    protected function geoTrianglesRotated()
    {
        ...
        for ($y = 0; $y <= 5; ++$y) {
            for ($x = 0; $x <= 5; ++$x) {
                ...
                $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xTriangleWidth, $t1) rotate($rotation, $halfTriangleWidth, $halfSideLength)")));

                // Add an extra one at top-right, for tiling.
                if ($y == 0) {
                    $yT1 = 6 * $sideLength * 0.5 - $sideLength / 2;
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xTriangleWidth, $yT1) rotate($rotation, $halfTriangleWidth, $halfSideLength)")));
                }
    ...
    protected function geoDiamonds()
    {
        ...
        for ($y = 0; $y <= 5; ++$y) {
            for ($x = 0; $x <= 5; ++$x) {
                ...
                $this->svg->addPolyline($diamond, array_merge($styles, array('transform' => "translate($t1, $t2)")));

                // Add an extra one at top-right, for tiling.
                if ($x == 0) {
                    $xT1 = 6 * $diamondWidth - $diamondWidth / 2 + $dx;
                    $xT2 = $diamondHeight / 2 * $y - $diamondHeight / 2;
                    $this->svg->addPolyline($diamond, array_merge($styles, array('transform' => "translate($xT1, $xT2)")));
                }
                ...
                if ($y == 0) {
                    ...
                    $this->svg->addPolyline($diamond, array_merge($styles, array('transform' => "translate($yT1, $yT2)")));
                }
                ...
                if ($x == 0 && $y == 0) {
                    ...
                    $this->svg->addPolyline($diamond, array_merge($styles, array('transform' => "translate($xyT1, $xyT2)")));
                }
            ...
            for ($x = 0; $x <= 3; ++$x) {
                if ($x % 2 == 0) {
                    if ($y % 2 == 0) {
                        ...
                        $this->drawInnerMosaicTile($x*$triangleSize*2, $y*$triangleSize*2, $triangleSize, array($this->hexVal($i, 1), $this->hexVal($i+1, 1)));
                    }
                } else {
                    if ($y % 2 == 0) {
                        $this->drawInnerMosaicTile($x*$triangleSize*2, $y*$triangleSize*2, $triangleSize, array($this->hexVal($i, 1), $this->hexVal($i+1, 1)));
    ...
    protected function geoPlaid()
    {
        ...
        while ($times++ <= 18) {
            ...
            $this->svg->addRectangle(0, $height, "100%", $stripeHeight, array(
                'opacity' => $opacity,
                'fill' => $fill,
            ));
            $height += $stripeHeight;
        ...
        while ($times++ <= 18) {
            ...
            $this->svg->addRectangle($width, 0, $stripeWidth, "100%", array(
                'opacity' => $opacity,
                'fill' => $fill,
            ));
            $width += $stripeWidth;
        ...
        for ($i = 0; $i <= 19; $i++) {
            ...
            switch ($i) {
                ...
                case 4: # left top / bottom triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($halfSideLength, $negativeHalfSideLength) rotate(0, $halfSideLength, $halfTriangleHeight)")));
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($halfSideLength, $tileHeightPlusHalfSideLength) rotate(0, $halfSideLength, $halfTriangleHeight) scale(1, -1)")));
                    break;
                case 5: # right top / bottom triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($tileWidthMinusHalfSideLength, $negativeHalfSideLength) rotate(0, $halfSideLength, $halfTriangleHeight) scale(-1, 1)")));
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($tileWidthMinusHalfSideLength, $tileHeightPlusHalfSideLength) rotate(0, $halfSideLength, $halfTriangleHeight) scale(-1, -1)")));
                    break;
                case 6: # center / top / right triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($halfTileWidthPlusHalfSideLength, $halfHexHeight)")));
                    break;
                case 7: # center / top / left triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($tileWidthMinusHalfTileWidthMinusHalfSideLength, $halfHexHeight) scale(-1, 1)")));
                    break;
                case 8: # center / bottom / right triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($halfTileWidthPlusHalfSideLength, $tileHeightMinusHalfHexHeight) scale(1, -1)")));
                    break;
                case 9: # center / bottom / left triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($tileWidthMinusHalfTileWidthMinusHalfSideLength, $tileHeightMinusHalfHexHeight) scale(-1, -1)")));
                    break;
                case 10: # left / middle triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($halfSideLength, $halfTileHeightMinusHalfSideLength)")));
                    break;
                case 11: # right / middle triangle
                    $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($tileWidthMinusHalfSideLength, $halfTileHeightMinusHalfSideLength) scale(-1, 1)")));
                    break;
                case 12: # left / top square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "translate($halfSideLength, $halfSideLength) rotate(-30, 0, 0)")));
                    break;
                case 13: # right / top square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(-1, 1) translate($negativeTileWidthPlusHalfSideLength, $halfSideLength) rotate(-30, 0, 0)")));
                    break;
                case 14: # left / center-top square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "translate($halfSideLength, $halfTileHeightMinusHalfSideLengthMinusSideLength) rotate(30, 0, $sideLength)")));
                    break;
                case 15: # right / center-top square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(-1, 1) translate($negativeTileWidthPlusHalfSideLength, $halfTileHeightMinusHalfSideLengthMinusSideLength) rotate(30, 0, $sideLength)")));
                    break;
                case 16: # left / center-top square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(1, -1) translate($halfSideLength, $negativeTileHeightPlusHalfTileHeightMinusHalfSideLengthMinusSideLength) rotate(30, 0, $sideLength)")));
                    break;
                case 17: # right / center-bottom square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(-1, -1) translate($negativeTileWidthPlusHalfSideLength, $negativeTileHeightPlusHalfTileHeightMinusHalfSideLengthMinusSideLength) rotate(30, 0, $sideLength)")));
                    break;
                case 18: # left / bottom square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(1, -1) translate($halfSideLength, $negativeTileHeightPlusHalfSideLength) rotate(-30, 0, 0)")));
                    break;
                case 19: # right / bottom square
                    $this->svg->addRectangle(0, 0, $sideLength, $sideLength, array_merge($styles, array('transform' => "scale(-1, -1) translate($negativeTileWidthPlusHalfSideLength, $negativeTileHeightPlusHalfSideLength) rotate(-30, 0, 0)")));
                    break;
            }
        }
    }
    ...
    protected function drawInnerMosaicTile($x, $y, $triangleSize, $vals)
    {
        ...
        $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTriangleSize, $y) scale(-1, 1)")))
            ->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTriangleSize, $yPlusTwoTriangleSize) scale(1, -1)")));
        ...
        $xPlusTriangleSize = $x + $triangleSize;
        $yPlusTwoTriangleSize = $y + $triangleSize * 2;
        $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTriangleSize, $yPlusTwoTriangleSize) scale(-1, -1)")))
            ->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTriangleSize, $y) scale(1, 1)")));
    ...
    protected function drawOuterMosaicTile($x, $y, $triangleSize, $val)
    {
        ...
        $this->svg->addPolyline($triangle, array_merge($styles, array('transform' => "translate($x, $yPlusTriangleSize) scale(1, -1)")))
            ->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTwoTriangleSize, $yPlusTriangleSize) scale(-1, -1)")))
            ->addPolyline($triangle, array_merge($styles, array('transform' => "translate($x, $yPlusTriangleSize) scale(1, 1)")))
            ->addPolyline($triangle, array_merge($styles, array('transform' => "translate($xPlusTwoTriangleSize, $yPlusTriangleSize) scale(-1, 1)")));
    }
